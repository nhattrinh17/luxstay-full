...
function Suggestions() {
    ...
    const [suggestionEnd, setSuggestionEnd] = useState(4)
    ...
    return(
        <div className="grid wide">
            <div className="container-suggestions">
                ...
                <button className={suggestionHead === 0 ? 
                        "container-suggestions__pre fas fa-chevron-left" : "container-suggestions__pre fas fa-chevron-left active"}
                        onClick={() => 
                            {
                    ...
                    <button className={suggestionEnd === suggestionsList.length ? 
                        "container-suggestions__next fas fa-chevron-right" : "container-suggestions__next fas fa-chevron-right active"}
                        onClick={() => 
                            {
                                setSuggestionEnd(pre => pre += 1)
                                setSuggestionHead(pre => pre += 1)
                            }}
                        disabled={suggestionEnd >= suggestionsList.length? true : false}>    
                        </button>
                    {
                        suggestionsList.map((suggestionsItem, index) => {
                            return(
                                <Link to={"/vi/suggestion/" + suggestionsItem.id} key={index}
                                    className={(index >= suggestionHead && index <= suggestionEnd)? 
