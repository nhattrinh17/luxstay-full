...
function Tutorial() {
    ...
    const [tutorialEnd, setTutorialEnd] = useState(3)
    ...
    return(
        <div className="grid wide">
            <div className="container-tutorial">
                ...
                <button className={tutorialHead === 0 ? 
                    "container-tutorial__pre fas fa-chevron-left" : "container-tutorial__pre fas fa-chevron-left active"}
                    onClick={() => 
                        {
                ...
                <button className={tutorialEnd === tutorialList.length ? 
                    "container-tutorial__next fas fa-chevron-right" : "container-tutorial__next fas fa-chevron-right active"}
                    onClick={() => 
                        {
                            setTutorialEnd(pre => pre += 1)
                            setTutorialHead(pre => pre += 1)
                        }}
                    disabled={tutorialEnd >= tutorialList.length? true : false}>    
                </button>
                    {tutorialList.map((tutorialItem, index) => {
                        return(
                            <div key={index} className={(index >= tutorialHead && index <= tutorialEnd)? 
                                ...
                                <Link to={"tutorial/" + tutorialItem.path} className="container-tutorial__link">
                                    <div className="container-tutorial__img" style={{backgroundImage:`url(${tutorialItem.img})`}}></div>
                                </Link>
                            </div>
